...
**Where these live.** Every token is published as `--color-<name>` — `--ground` in the tables above is
`--color-ground` in CSS. Three files are generated from `scripts/palette.py` and committed:
`packages/ui/src/tokens/palette.generated.ts` (the typed hex mirror),
`packages/ui/src/tokens/tokens.css` (custom properties, light and dark) and
`packages/ui/src/tokens/tailwind.css` (the Tailwind v4 theme, which clears Tailwind's own palette with
`--color-*: initial`). `@berelax/pdf` inlines the same tokens into every invoice, so a document and the
page that produced it cannot be different greys.

**Three gates, all in `pnpm verify`.** `pnpm palette` re-derives every token, re-measures it, and fails
if the generated module or *these tables* disagree with it. `pnpm tokens` fails if a generated
stylesheet is stale. `pnpm colours` rejects an un-tokened colour anywhere outside the token layer, the
decorative gold on anything text-bearing, and any Tailwind default palette utility. See
[ADR 0012](adr/0012-design-tokens.md).
