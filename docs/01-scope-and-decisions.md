...
| 19b | Prepaid products | **Packages only**, defined as `package_template` rows configured in settings from day one. A **deposit is not a prepaid product** — it is a part-payment against one specific booking, with no balance, no expiry and no redemption schedule, so it does not reintroduce a second deferred-revenue path | One prepaid product means one deferred-revenue path, one liability account, one migration artefact and one VAT date-of-supply question to settle with the accountant. Vouchers and memberships each add a distinct recognition pattern; declining both now is a real simplification, and adding one later is a new product rather than a variant |
| 21 | Treatment style | **A service is the pair `(style × treatment)`** — Asian/Arabic is a treatment-style attribute, not a therapist one | Confirmed by the owner. Keeps pricing and therapist assignment **decoupled**: the price is known at service selection and does not change when the therapist changes. Style still maps to a required therapist **skill** for eligibility. 8 services × 4 durations = 32 variants, with no third pricing axis |
| 22 | Business day | **`business_day` is first-class**, not derived from the calendar date | Trading hours are 11:00–02:00, so a 01:30 appointment belongs to the *previous* business day for cash-up, rota, commission and every daily report. A naive open/close comparison also fails outright. Retrofitting this means rewriting every report |
| 23 | Therapist publishing | A therapist page is publishable **only with a display name and a recorded photography consent** | 19 therapists currently have photos and no names. Publishing without the guard yields 19 indexed, empty, near-duplicate pages — worse for SEO than none. Unnamed therapists render as unlinked photo cards |
| 24 | Analytics | **A first-party event store in our own Postgres is the source of truth** for business reporting; GA4 and Meta are for the ad platforms only | Non-sampled, ad-blocker resilient, joinable to completed and *paid* bookings, and it needs no third-party script. Also retroactively justifies skipping a server-side GTM container |
| 25 | WhatsApp attribution | A **short reference code in the prefilled WhatsApp message**, pasted into the booking by staff | WhatsApp is the real booking channel, and the conversation is invisible to us. The ref is the only way to attribute a paid appointment back to a Meta or Google click. Ref-capture rate is tracked as a data-quality metric, and if it is low the funnel says so rather than fabricating the join |
| 26 | Banned-claims lexicon | **One lexicon, owned by `regulatory_profile`**, consumed by the CMS publication lint, the service display-name lint, the message-template lint and the SEO agent's keyword filter | Decomposition found it required in three places and built by none. A single source prevents the situation where a word is blocked in page copy and permitted in a service name — which is the version an inspector would notice |
| 27 | Document rendering | **Generated PDFs are HTML rendered in headless Chromium**, with the Latin and Arabic faces embedded in the file | Arabic needs contextual shaping (OpenType GSUB) and UAX #9 reordering; a PDF library gives neither, and hand-rolling them is not a job for a one-person build. Costs the worker a ~170MB Chromium binary. Embedding the fonts also removes the failure where a missing Arabic face renders notdef boxes whose ToUnicode map still reports the right characters — so a test asserting "the Arabic is there" passes while every glyph is a box. [ADR 0011](adr/0011-documents-render-in-chromium.md) |
| 28 | Bidirectional text | **Every Latin run inside Arabic is isolated unconditionally**, at the interpolation site, in documents and in outbound messages alike | Measured, not predicted: `+971 52 823 9069` renders as `9069 823 52 971+`, and `11:00 - 02:00` renders as `02:00 - 11:00` — the trading hours silently swap and the sentence still reads plausibly. A bare reference happens to survive, which is exactly why isolation is not applied case by case: the same string is safe in one sentence and broken in the next |
| 29 | Untrusted text | **Bidi format controls are stripped from every interpolated value**, and literal invisible characters are rejected in source by `pnpm invisibles` | A U+202E in a customer name reverses the rest of the line on a document that states an amount. The same hazard in source is CVE-2021-42574, where reviewed code differs from compiled code |
| 30 | Palette | **The palette is derived by `scripts/palette.py`, not chosen**, and the generated token module, `tokens.css` and the Tailwind theme are committed and drift-gated | Ratios are true on the day they are measured. A hex nudged by eye leaves the system claiming 4.62:1 while shipping 3.8:1, and nothing catches it. Committing the generated files puts a colour change in the diff and gives the web, the emails and the PDFs one set of bytes to agree on. [ADR 0012](adr/0012-design-tokens.md) |
| 31 | Brand gold | **`#C08A43` is decorative only and may never carry text, an icon that means something, or a border that conveys state** — enforced by `pnpm colours` | It measures 2.90:1 in light mode, below both the 4.5:1 body threshold and the 3:1 user-interface threshold. `--color-accent-gold` at 4.62:1 carries text and still reads as gold. The same `#C08A43` reaches 6.19:1 on the dark ground and is the primary accent there — accent polarity flips with the theme |
| 32 | External providers | **Every external service sits behind a port with a fake that fails on demand, selection is configuration, and no fake returns success without writing to a visible call log.** A `real` provider resolves to a named adapter that refuses loudly rather than falling back to the fake | A stub that returns success is worse than no stub: it makes a broken system demo perfectly. A silent fallback is worse still — a production deploy that looks connected and sends nothing. The fakes also mean the system is buildable and demoable with zero credentials, which matters most for Google Business Profile, where access is granted by application review and the wait is measured in weeks. [ADR 0022](adr/0022-provider-ports-and-fakes.md) |
