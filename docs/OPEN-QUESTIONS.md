...
| Y11-blocked-vat | **Which of this business's costs fall in a blocked input VAT category?** Two are unclear and both are incurred monthly: (a) **staff accommodation and transport** — an employee benefit is recoverable only where the business is obliged to provide it, docs/04 §7 names mandatory unemployment and health insurance and names no obligation to house or transport staff, while docs/13 §2 says transport at 02:00 "is a safety matter"; (b) **complimentary in-treatment refreshments** — an entertainment service, or a consumable used in making a taxable supply? | **Both blocked (not recoverable)**, the conservative reading: an over-claim is a penalty and an under-claim is money left on the table, and only one of those is a compliance failure. 5060 carries `vat_box = blocked_input_tax` (migration 0034) and refreshments are coded to 6090. Answering it is an audited reclassification (`reclassifyAccountRecoverability`) that applies to the next bill and cannot reach a filed period. No motor-vehicle category exists: the business operates no vehicle (docs/01) | open | recoverable input VAT in every VAT return |
