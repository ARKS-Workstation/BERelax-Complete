...
describe('acceptance — with NO app_setting row, the database still enforces strict matching', () => {
  it('reads strict from an empty settings table and offers no cross-gender slot', async () => {
    ...
    //
    // The KEYS as they were before the probe, so the claim at the bottom is *this file changed nothing*
    // rather than *the table is non-empty*. It asserted `count(*) > 0` until G-CONN-09's run found it
    // failing on a virgin database: no migration seeds `app_setting`, so on the first run against a fresh
    // database the count is legitimately zero before the probe and the assertion fails — and on every
    // later run some earlier suite has left rows behind and it passes. That is the brief's rule 12 shape
    // exactly, and rule 9's: assert a delta, never a total.
    const keysBefore = (await sql<{ key: string }[]>`select key from app_setting order by key`).map(
      (row) => row.key,
    )
    ...
    // The rollback happened, so this file leaves the settings table exactly as it found it — key for key,
    // which is a stronger claim than a count and one that holds whether or not anything had seeded it.
    const keysAfter = (await sql<{ key: string }[]>`select key from app_setting order by key`).map(
      (row) => row.key,
    )
